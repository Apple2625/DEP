...
def tra(model, dataset, data, time_step, epoch, optimizer, lr_scheduler, scaler, loss_lambda=0.0, sl = 0.05, attacker=None, writer=None):
...
def DEP_projection_update(grad):
    if grad.dim() < 2:
        return grad
    else:
        ...
        try:
            ...
            try:
                ...
                return grad
        dominant_eigencomponent = S[0] * torch.outer(U[:, 0], Vh[0, :])
        new_grad_mat = grad_mat - dominant_eigencomponent
...
def DEP(model):
    for param in model.parameters():
        if param.grad is not None:
            param.grad.data.copy_(DEP_projection_update(param.grad.data))
